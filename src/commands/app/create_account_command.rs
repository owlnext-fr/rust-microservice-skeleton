...
use crate::{
    ...
    domain::model::{account::NewAccount, application::NewApplication, user::NewUser},
    middlewares::{
...
/// a command to create account + apploication + user.
...
#[async_trait]
impl ConsoleCommand for CreateAccountCommand {
    fn get_name(&self) -> String {
        "app:create-account".into()
    }
    ...
    async fn do_run(&self, _args: &HashMap<String, Option<String>>) -> Result<CommandResult> {
        ...
        io.title("Account structure creation");

        io.comment(
            "This command will guide you through account, application and first user creation",
        );
        io.new_line();
        ...
        let account_name = io.ask_question("Name of the account:");
        ...
        io.step(1, 3, &format!("Creating account {account_name}..."));
        let account = self
            .account_middleware
        ...
        data.push(("Account name", account.name));
